...
import bbs.com.xinfeng.bbswork.databinding.UpgradeDialogLayoutBinding;
...
public class UpgradeUtil {
    ...
    private UpgradeDialogLayoutBinding mBinding;
    ...
    private void showUpdataDialog(Activity context, UpgradeBean bean) {
        if (mDialog != null && mDialog.isShowing())
            mDialog.dismiss();
        android.app.AlertDialog.Builder builder = new android.app.AlertDialog.Builder(context, R.style.MyDialog);
        View view = View.inflate(context, R.layout.upgrade_dialog_layout, null);
        mBinding = DataBindingUtil.bind(view);
        mBinding.txtPointContent.setText(bean.desc);
        mBinding.txtPointTitle.setText("发现新版本 V" + bean.getVersion_name());
        if (isForce) {
        ...
        mBinding.txtButtonRight.setText("立即升级");
    ...
    private void updateNormal(Activity context, UpgradeBean versionBean) {
        if (!installApk(context, versionBean)) {
            ...
            if (mBinding != null && isForce) {
                mBinding.txtButtonRight.setClickable(false);
                mBinding.txtButtonRight.setText("升级中...0%");
            } else {
                mDialog.dismiss();
            }
        }
    ...
    public void updataProgress(Progress p) {
        if (mBinding != null) {
            if (p.isError) {
                ...
                mBinding.txtButtonRight.setClickable(true);
            }
            mBinding.txtButtonRight.setText("升级中..." + p.progress + "%");
            if (p.progress == 100) {
                mBinding.txtButtonRight.setClickable(true);
